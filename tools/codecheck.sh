...
source_dirs="$src_dir $SCRIPT_DIR"

if [ -d "$examples_dir" ]; then
	source_dirs="$source_dirs $examples_dir"
fi


echo "running black"
black --line-length=120 $source_dirs
exit_code=$?
...
pycodestyle --show-source --statistics --count --ignore=$ignore_errors $source_dirs
exit_code=$?
...
echo "running flake8"
python3 -m flake8 --show-source --statistics --count --ignore=$ignore_errors $source_dirs
exit_code=$?
...
#echo "to ignore warning for one line put following comment in end of line: # nosec
bandit --skip "${skip_list}" -r $source_dirs -x "$src_dir/test*"
exit_code=$?
if [ $exit_code -ne 0 ]; then
    exit $exit_code
fi
